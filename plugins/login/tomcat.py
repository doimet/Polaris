...
class Plugin(Base):
    ...
    @cli.options('port', desc="设置目标端口", type=int, default=8080)
    ...
    def url(self, ip, port, method, username, password, timeout, workers) -> dict:
    ...
    async def custom_task(self, ip, port, username, password, timeout):
        ...
        r = await self.async_http(
            method='get',
            url=urllib.parse.urljoin(ip, './manager/html'),
            headers={
